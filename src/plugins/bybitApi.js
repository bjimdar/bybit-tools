      ...
      methods: {
              ...
              case "order":
                for (let i = 0; i < data.data.length; i++) {
                  if (data.data[i].symbol === this.currentSymbol) {
                    if (
                      data.data[i].order_status === "Cancelled" ||
          ...
          if (data.type === "delta") {
            if (data.data.update[0].last_price_e4) {
              this.lastPrice = Number(
                data.data.update[0].last_price_e4 + "e-4"
              ).toFixed(2);
            }
            if (data.data.update[0].mark_price_e4) {
              this.markPrice = Number(
                data.data.update[0].mark_price_e4 + "e-4"
              ).toFixed(2);
            }
          }
        },
        ...
        async getPosition() {
          try {
            ...
            let res = await axios.get(this.url + "position/list", options);
            if (res.data.ret_msg === "ok") {
              ...
              this.walletBalance = res.data.result.filter(
                pos => pos.symbol === this.currentSymbol
              )[0].wallet_balance;
              this.openPosition = res.data.result.filter(
                pos => pos.symbol === this.currentSymbol && pos.size > 0
              )[0];
            } else {
              console.error(res);
              this.$notify({
        ...
        async placeOrder(data) {
          try {
            let res = await axios.post(
              this.url + "v2/private/order/create",
              this.signData(data)
            );
            console.log(res);
            if (res.data.ret_msg === "OK") {
        ...
        async cancelOrder(id) {
          try {
            ...
            let res = await axios.post(
              this.url + "v2/private/order/cancel",
              this.signData(data)
            );
            if (res.data.ret_msg === "OK") {
              this.$notify({
        ...
        async cancelAllOpenOrders() {
          try {
            ...
            let res = await axios.post(
              this.url + "v2/private/order/cancelAll",
              this.signData(data)
            );
            if (res.data.ret_msg === "OK") {
              this.$notify({
