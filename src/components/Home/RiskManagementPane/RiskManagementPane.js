export default {
  ...
  computed: {
    table: function() {
      return [
        ...
        },
        {
          ...
          value: Math.max(
            this.contractsToTrade / this.walletBalanceUSD,
            1
          ).toFixed(2)
        },
        {
        ...
        },
        {
          ...
          value:
            (
              (100 * (this.targetProfitUSD - this.fees)) /
              this.contractsToTrade
            ).toFixed(2) + "%"
        },
        {
          label: "Risk | Reward",
          value: (
            (this.targetProfitUSD - this.fees) /
            (this.riskAmount + this.fees)
          ).toFixed(2)
        }
      ];
    },
    ...
    contractsToTrade: function() {
      return Math.round(
        (this.percent(this.risk) * this.walletBalanceUSD) /
    ...
    fees: function() {
      if (this.order.orderType === "limit") {
        return (
          this.contractsToTrade * 0.00075 - this.contractsToTrade * 0.00025
        );
      } else {
