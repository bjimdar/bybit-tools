  ...
  computed: {
    ...
    tpProfit: function() {
      if (this.form.takeProfit && this.form.contracts) {
        let profit =
          Math.abs(
            1 / this.$bybitApi.lastPrice - 1 / parseFloat(this.form.takeProfit)
          ) * this.form.contracts;
        return (
          profit.toFixed(4) +
          " ≈ " +
    ...
    slLoss: function() {
      if (this.form.stopLoss && this.form.contracts) {
        let loss =
          Math.abs(
            1 / this.$bybitApi.lastPrice - 1 / parseFloat(this.form.stopLoss)
          ) *
            this.form.contracts +
  ...
  watch: {
    form: {
      ...
      handler: async function() {
        if (this.active && this.form.stopLoss && this.form.takeProfit) {
          await this.$nextTick();
          if (this.$refs.form.validate()) {
            this.$emit("order", {
