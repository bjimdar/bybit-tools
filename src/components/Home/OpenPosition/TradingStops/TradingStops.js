  ...
  computed: {
    ...
    tpProfit: function() {
      if (
        this.takeProfit &&
        this.takeProfit != 0 &&
        this.$bybitApi.openPosition.size
      ) {
        let profit =
          Math.abs(
            1 / this.$bybitApi.openPosition.entry_price -
              1 / parseFloat(this.takeProfit)
          ) * this.$bybitApi.openPosition.size;
        return (
          profit.toFixed(4) +
          " ≈ " +
          (profit * this.$bybitApi.openPosition.entry_price).toFixed(2) +
          "USD"
        );
      }
    },
