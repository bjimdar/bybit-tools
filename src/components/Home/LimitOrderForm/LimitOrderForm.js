  ...
  computed: {
      ...
      return {
        priceRules: [
          ...
          v =>
            (v &&
              Number(v + "e4") %
                Number(this.$bybitApi.currentTickSize + "e4") ===
                0) ||
            "Price must be a multiple of " + this.$bybitApi.currentTickSize
        ],
        takeProfitRules: [
          ...
          v =>
            ...
            (v &&
              Number(v + "e4") %
                Number(this.$bybitApi.currentTickSize + "e4") ===
                0) ||
            "Stop Loss must be a multiple of " + this.$bybitApi.currentTickSize
        ],
        contractsRules: [
          ...
          v =>
            (v &&
              Number(v + "e4") %
                Number(this.$bybitApi.currentQtyStep + "e4") ===
                0) ||
            "Quantity must be a multiple of " + this.$bybitApi.currentQtyStep
        ]
      };
    },
    tpProfit: function() {
      if (this.form.price && this.form.takeProfit && this.form.contracts) {
        let profit =
          Math.abs(1 / this.form.price - 1 / parseFloat(this.form.takeProfit)) *
          this.form.contracts;
        return (
          profit.toFixed(4) +
          " ≈ " +
          (profit * this.$bybitApi.lastPrice).toFixed(2) +
          "USD"
        );
      }
    },
    slLoss: function() {
      if (this.form.price && this.form.stopLoss && this.form.contracts) {
        let loss =
          Math.abs(1 / this.form.price - 1 / parseFloat(this.form.stopLoss)) *
            this.form.contracts +
